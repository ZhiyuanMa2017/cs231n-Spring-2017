...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        x_testbatchnorm = (x- running_mean)/np.sqrt(running_var+eps)
        out = gamma*x_testbatchnorm+beta
        #######################################################################
        #                          END OF YOUR CODE                           #
        #######################################################################
    else:
        ...
        for f in range(F):
            for i in range(H_):
                for j in range(W_):
                    ...
                    dx_pad[n,:,i*s:i*s+HH,j*s:j*s+WW] += dout[n,f,i,j]*w[f,:]
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(h_new):
                for j in range(w_new):
                    pool = x[n,c,i*s:i*s+ph,j*s:j*s+pw]
                    dx[n,c,i*s:i*s+ph,j*s:j*s+pw] = (pool==np.max(pool))*dout[n,c,i,j]
